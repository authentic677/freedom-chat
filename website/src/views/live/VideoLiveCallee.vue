...
export default {
    ...
    data(){
        return {
            ...
            status:0, //0表示用户还未点击接听按钮，1表示已经在接听了
        }
    },
    props:['peerUser'],
    methods:{
        ...
        answer(){
            this.$store.state.ws.send(JSON.stringify({
                command: 'video-response',
                caller:this.$route.params.uid,
                status:'agree',
                id:this.localId
            }))
        }
...
<template>
    <div class="video-live-callee">
        <div class="top">
            {{status===0? '对方邀请进行视频通话':'正在通话中' }}
        </div>
        <div class="middle">
            ...
            <div class="info">
                <img :src="peerUser.avatar">
                <div class="text">
                    <div class="title">{{peerUser.username}}</div>
                    <div class="description">请求与你进行视频通话</div>
                </div>
            </div>
        </div>
        <div class="bottom">
